...
#       print('Ranking3: '+str(datetime.datetime.now()))
    ...
    for doc, curDocVec in docVectors.items():
        docVectors[doc] = np.multiply(curDocVec, queryVector)
    # docScores=[docVectors[doc] = [np.multiply(curDocVec, queryVector)] for doc, curDocVec in docVectors.items()]
    ...
    for termIndex, (term, freq) in enumerate(queryDict.items()):
      queryDict[term] = freq/len(terms)
      queryDict[term] = queryDict[term] * self.idf[term]

    for termIndex, term in enumerate(terms):
        queryVector[termIndex] = queryDict[term]

    docScores=[[self.cosineSimilarity(curDocVec, queryVector), doc] for doc, curDocVec in docVectors.items()]
    print('Ranking4')
    print(datetime.datetime.now())
#     print(docScores)
